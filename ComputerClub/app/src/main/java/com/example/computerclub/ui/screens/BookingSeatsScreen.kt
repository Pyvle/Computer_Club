...
fun BookingSeatsScreen(
    ...
    val durationMin = Duration.between(startDt, endDt).toMinutes().coerceAtLeast(0)
    ...
    Column(
        ...
        Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.spacedBy(12.dp)) {
            Column(Modifier.weight(1f)) {
                ...
                val header = if (startDate == endDate) {
                    "${formatDateShort(startDate)} • ${formatTime(draft.startMin)}–${formatTime(draft.endMin)} • ${formatDuration(durationMin.toInt())}"
                } else {
                    "${formatDateShort(startDate)} ${formatTime(draft.startMin)} – ${formatDateShort(endDate)} ${formatTime(draft.endMin)} • ${formatDuration(durationMin.toInt())}"
                }
