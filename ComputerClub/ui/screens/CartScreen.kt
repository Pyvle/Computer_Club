...
fun CartScreen(appVm: AppViewModel) {
    ...
    val bookingCost = remember(booking.startMin, booking.endMin, booking.selectedSeatIds) {
        ...
        if (end <= start) 0 else {
            ...
            selected.sumOf { seat ->
                val rate = if (seat.type == com.example.computerclub.model.SeatType.VIP) 350 else 200
                rate * hours
            }
        }
    }
    ...
    Column(Modifier.fillMaxSize().padding(16.dp), verticalArrangement = Arrangement.spacedBy(12.dp)) {
        ...
        if (booking.selectedSeatIds.isNotEmpty()) {
            val start = appVm.minToLabel(booking.startMin)
            val end = appVm.minToLabel(booking.endMin)
